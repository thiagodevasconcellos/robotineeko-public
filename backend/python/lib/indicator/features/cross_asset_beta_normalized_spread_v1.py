...
class CrossAssetBetaNormalizedSpreadV1(Calculator):
    """
    Cross-asset spread surface that normalizes the level spread by a rolling
    hedge beta estimated on aligned returns.

    The indicator exposes a level-style spread shell instead of the return
    residual shell used in paper 61. It keeps the dynamic hedge relation
    visible through:
    - hedge_beta, hedge_beta_smooth, beta_stability
    - beta_adjusted_peer_return for coherence gating
    - beta_normalized_spread: log-price spread adjusted by dynamic beta
    - spread_zscore and absolute_spread_zscore
    - spread_delta and spread_velocity for directional entry or decay checks
    - agreement and sync_score on the aligned return leg
    """
    ...
    def __init__(
        ...
        safe_spread_window = max(3, int(spreadWindow or 3))
        ...
        peer_context = cross_asset_confirmation_module.ensure_market_data(
            ...
            source='cross_asset_beta_normalized_spread_v1_indicator',
        )
        if not peer_context.get('ready'):
            raise ValueError(
                f'CrossAssetBetaNormalizedSpreadV1 requires ready peer data for {safe_peer_symbol} {symbol.timeframe}.'
            )
        ...
        primary_close = aligned['primary_close'].clip(lower=1e-9)
        peer_close = aligned['peer_close'].clip(lower=1e-9)

        primary_return = primary_close.pct_change(safe_lookback, fill_method=None)
        ...
        adjusted_peer_log = np.log(peer_close) * relation_sign
        beta_normalized_spread = primary_log - (hedge_beta_smooth * adjusted_peer_log)
        spread_mean = beta_normalized_spread.rolling(window=safe_spread_window, min_periods=3).mean()
        spread_std = beta_normalized_spread.rolling(window=safe_spread_window, min_periods=3).std(ddof=0)
        spread_std = spread_std.replace(0.0, np.nan)
        spread_zscore = (beta_normalized_spread - spread_mean).divide(spread_std)
        absolute_spread_zscore = spread_zscore.abs()
        spread_delta = beta_normalized_spread.diff()
        spread_velocity = spread_zscore.diff()
        ...
        spread_scale = absolute_spread_zscore.divide(3.0).clip(lower=0.0, upper=1.0)
        sync_score = pd.Series(0.0, index=aligned.index, dtype=float)
        coherent_mask = agreement > 0.0
        sync_score.loc[coherent_mask] = (1.0 - spread_scale.loc[coherent_mask]).clip(lower=0.0, upper=1.0)
        ...
        symbol.add_feature(f'{self.name}_beta_normalized_spread', beta_normalized_spread)
        ...
        symbol.add_feature(f'{self.name}_absolute_spread_zscore', absolute_spread_zscore)
        symbol.add_feature(f'{self.name}_spread_delta', spread_delta)
        symbol.add_feature(f'{self.name}_spread_velocity', spread_velocity)
