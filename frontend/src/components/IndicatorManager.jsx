...
function buildIndicatorLogText(action, indicator) {
    ...
    return `${action} feature ${normalized.name} with alias ${aliasText} (${paramsText}).`
}
...
export function IndicatorManager({
    ...
    const normalizedSettings = useMemo(
        () => normalizeChartSettings(chartSettings),
        [chartSettings]
    )
    ...
    const existingAliases = useMemo(
        () => normalizedSettings.indicators.map((indicator) => indicator.alias).filter(Boolean),
        [normalizedSettings.indicators]
    )

    const existingPaneIds = useMemo(() => {
        const paneIds = normalizedSettings.indicators
    ...
    const selectedCurrentId = normalizedSettings.indicators.some((indicator) => indicator.id === selectedCurrentIdDraft)
        ? selectedCurrentIdDraft
        : (normalizedSettings.indicators[0]?.id || '')

    const selectedDefinition = useMemo(
        () => getIndicatorDefinition(selectedIndicatorName),
        [selectedIndicatorName]
    )
    const selectedCurrentIndicator = useMemo(
        () => normalizedSettings.indicators.find((indicator) => indicator.id === selectedCurrentId) || null,
        [normalizedSettings.indicators, selectedCurrentId]
    )
    const selectedCurrentDefinition = useMemo(
        () => getIndicatorDefinition(selectedCurrentIndicator?.name),
        [selectedCurrentIndicator?.name]
    )
    ...
    function handleAddIndicator(nextIndicator) {
        const nextNumber = normalizedSettings.indicators.filter((indicator) => indicator.name === nextIndicator.name).length + 1
        const normalizedIndicator = normalizeIndicator({
            ...nextIndicator,
        ...
        onChange({
            ...normalizedSettings,
            indicators: [...normalizedSettings.indicators, normalizedIndicator],
        })

        onLogEvent?.(buildIndicatorLogText('Added', normalizedIndicator))

        setActiveTab('current')
        setSelectedCurrentIdDraft(normalizedIndicator.id)
    }
    ...
    function handleUpdateIndicator(nextIndicator) {
        ...
        const updatedIndicator = normalizeIndicator({
            ...selectedCurrentIndicator,
            ...nextIndicator,
            id: selectedCurrentIndicator.id,
        }, normalizedSettings.indicators.findIndex((indicator) => indicator.id === selectedCurrentIndicator.id))

        onChange({
            ...normalizedSettings,
            indicators: normalizedSettings.indicators.map((indicator, index) => (
        ...
        onLogEvent?.(buildIndicatorLogText('Updated', updatedIndicator))
    }
    ...
    function handleRemoveIndicator() {
        ...
        const updatedIndicators = normalizedSettings.indicators.filter((indicator) => indicator.id !== selectedCurrentIndicator.id)
        ...
        onLogEvent?.(`Removed feature ${selectedCurrentIndicator.alias || selectedCurrentIndicator.name}.`)

        setSelectedCurrentIdDraft(updatedIndicators[0]?.id || '')
    }

    return (
        <div className='overlayContainer indicatorManagerOverlay'>
            <div className='fog' onClick={onClose} />

            <div className='overlay indicatorManagerWindow'>
                    ...
                    <aside className='indicatorManagerSidebar'>
                        ...
                        {activeTab === 'add' && (
                            <div className='indicatorManagerList indicatorManagerCatalog'>
                                ...
                                {catalogListTab === 'favorites' && !favoriteFeatureNames.length ? (
                                    <div className='indicatorManagerEmptyList'>
                        ...
                        {activeTab === 'current' && (
                            <div className='indicatorManagerList'>
                                {normalizedSettings.indicators.length === 0 && (
                                    <div className='indicatorManagerEmptyList'>
                                        No features added yet.
                                    </div>
                                )}

                                {normalizedSettings.indicators.map((indicator) => (
                                    <button
                                        type='button'
                                        key={indicator.id}
